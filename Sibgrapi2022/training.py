...
np.random.seed(seed)
try:
    tf.random.set_seed(seed) # tensorflow 2
except:
    tf.set_random_seed(seed) # tensorflow 1
...
def load_model(model, path, device_ids):
    ...
    early_stopping = EarlyStopping(best_loss=best_loss)
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, SR, HR):
        ...
        if self.OCR_path != False:
            for imgSR, imgHR in zip(SR, HR):
                ...
                if self.OCR is not None:
                    predSR = self.OCR_pred(imgSR)[0].rstrip('#')
                    predHR = self.OCR_pred(imgHR)[0].rstrip('#')
        ...
        distHRSR = (1/(10**(MSE_zeros+2)))*(distHRSR/count)
...
@EventlogHandler
def fit(model, train_dataloader, optimizer, criterion, device, epoch):
    print('TRAINING')
    model.train()
    ...
    with torch.no_grad():
        with tqdm(enumerate(test_dataloader), total=len(test_dataloader)) as prog_bar:
            for i, batch in enumerate(prog_bar):
                ...
                prog_bar.set_postfix(loss=loss.detach().item())
...
def main():
    ...
    for epoch in range(current_epoch, 50):
        ...
        train_epoch_loss = fit(model, train_dataloader, optimizer, criterion, device_ids[0], epoch)
